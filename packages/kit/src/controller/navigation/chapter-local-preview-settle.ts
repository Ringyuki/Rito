...
export function settleChapterLocalIncoming(
  ...
  if (event.direction !== active.direction || event.targetSpread !== active.mountSpreadIndex) {
    fatalChapterLocalContainment(state, deps, active, 'preview settle identity mismatch');
    return true;
  }
  if (!event.committed) {
  ...
  if (!deps.pool.commitProvisionalStage(active.stageToken)) {
    fatalChapterLocalContainment(state, deps, active, 'preview stage commit failed');
    return true;
  }
  ...
  if (active.termination) {
    ...
    promoteOrSettleChapterLocalVisual(state, deps, active);
  } else {
...
function promoteOrSettleChapterLocalVisual(
  ...
  if (promoteChapterLocalExact(state, deps, active)) return;
  if (
...
export function settleChapterLocalRollback(
  ...
  if (
    ...
    fatalChapterLocalContainment(state, deps, active, 'preview rollback settle identity mismatch');
    return true;
  }
  if (!deps.pool.completeProvisionalRollback(active.stageToken)) {
    fatalChapterLocalContainment(state, deps, active, 'preview rollback promotion failed');
    return true;
  }
  tryFinishRestoredMount(state, deps, active);
  return true;
...
export function startChapterLocalTermination(
  ...
  if (active.phase === 'animating') {
    ...
    startRollback(state, deps, active);
...
export function promoteChapterLocalExact(
  ...
  if (exact.spreadIndex === current) {
    return promoteSameSpreadExact(state, deps, active, exact);
  }
  ...
  if (!deps.pool.promoteProvisionalExact(active.stageToken, slot, exact.spreadIndex)) {
    fatalChapterLocalContainment(state, deps, active, 'exact preview promotion failed');
    return false;
  }
...
function promoteSameSpreadExact(
  ...
  if (!deps.pool.finishSameSpreadProvisionalExact(active.stageToken, active.mountSpreadIndex)) {
    fatalChapterLocalContainment(state, deps, active, 'same-spread exact preview promotion failed');
    return false;
  }
  finishChapterLocalLease(active);
  completeExactPromotion(state, deps, active, exact);
