...
import type { Page, Rect } from '../layout-types';
...
function buildSearchEngineApi(s: SearchState): SearchEngine {
  return {
    ...
    search(query, options) {
      if (!s.index) return;
      setResults(s, searchIndex(s.index, query, options));
    },
    setResults(results) {
...
function setResults(s: SearchState, results: readonly SearchResult[]): void {
  s.results = results;
