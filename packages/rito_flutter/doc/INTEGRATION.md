...
| `peek(from:, requestId:, direction:, work:)`                                                                | Read-only neighbor fetch: returns the adjacent page as a fully prepared artifact **without changing the visible state**, or null when the neighbor would need layout work (UI falls back to fade-in). The artifact counts against the live budget and must be released. |
