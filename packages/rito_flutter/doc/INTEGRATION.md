...
| `peek(from:, requestId:, direction:, work:)`                                                                | Read-only neighbor fetch: returns the adjacent page as a fully prepared artifact **without changing the visible state**. The engine paginates toward the neighbor within the work budget (chapter-local and publication-backed pages alike); null only when it is still out of reach (chapter/book boundary or budget exhausted; UI falls back to fade-in). The artifact counts against the live budget and must be released. |
