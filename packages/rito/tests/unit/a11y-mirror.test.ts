...
describe('createA11yMirror', () => {
  ...
  it('does not create anchors for dangerous schemes', () => {
    const mirror = createA11yMirror(document.body);
    mirror.update([
      { role: 'link', text: 'unsafe', href: 'java\nscript:alert(1)', bounds, children: [] },
    ]);
    expect(mirror.container.querySelector('a')).toBeNull();
    expect(mirror.container.textContent).toBe('unsafe');
  });
});

describe('sanitizeA11yHref', () => {
  it('allows EPUB-relative and common external links', () => {
    expect(sanitizeA11yHref('../chapter.xhtml#part')).toBe('../chapter.xhtml#part');
    expect(sanitizeA11yHref('https://example.com')).toBe('https://example.com');
    expect(sanitizeA11yHref('mailto:reader@example.com')).toBe('mailto:reader@example.com');
  });

  it('rejects executable and embedded-content schemes', () => {
    expect(sanitizeA11yHref('javascript:alert(1)')).toBeUndefined();
    expect(sanitizeA11yHref('java\tscript:alert(1)')).toBeUndefined();
    expect(sanitizeA11yHref('data:text/html,<script></script>')).toBeUndefined();
  });
});
