...
import type { TextMeasurer } from '../../ts-core/layout/text/text-measurer';
import { buildSearchIndex, search as searchIndex } from '../../ts-core/interaction/search';
import type { SearchOptions, SearchResult } from '../../ts-core/interaction/search';
import { disposeAssets } from '../../ts-core/render/web';
import { getSpreadDimensions } from '../../ts-core/render/spread';
...
type ReaderMethods = ReturnType<typeof buildRenderMethods> &
  ...
  ReturnType<typeof buildSearchMethods> &
...
function buildRenderMethods(
  ...
  return {
    ...
    renderSpreadTo: (
      ...
      return renderSpreadToContext(state, targetCtx, index);
    },
...
function buildSearchMethods(state: ReaderState) {
  return {
    search(query: string, options?: SearchOptions): readonly SearchResult[] {
      return searchIndex(buildSearchIndex(state.resources.pages), query, options);
    },
  };
}
