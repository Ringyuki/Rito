...
export type PreparedChapterStyleContext = Pick<
  PreparedPaginationContext,
  'rules' | 'rulesByStylesheet' | 'rootFontSize'
>;

export interface ResolvedChapterStyleTree {
  readonly ancestors: readonly SelectorTarget[];
  readonly chapterBodyStyle: ComputedStyle;
  readonly rules: readonly CssRule[];
  readonly styled: readonly StyledNode[];
}
...
export function buildChapterStyleTree(
  nodes: readonly DocumentNode[],
  config: LayoutConfig,
  context: PreparedChapterStyleContext,
  ...
  return {
    ...
    chapterBodyStyle,
    rules,
    styled: forceTypographyOnTree(cascaded, config),
  };
}
