...
function createCanvasRenderState(options: CanvasDisplayListOptions | undefined): CanvasRenderState {
  if (options?.imageResolver) {
    ...
    return { resolveImage: (src: string) => imageResolver.resolveImage(src) };
  }
  const resolveImage = options?.images ? buildHrefResolver(options.images) : () => undefined;
  return { resolveImage };
}
...
function renderCommand(
  ...
  switch (command.kind) {
    ...
    case 'paintBlock':
      ...
      break;
    case 'paintText':
      paintText(ctx, command);
      break;
    case 'paintRuby':
      paintRuby(ctx, command);
      break;
    case 'paintImage':
...
function paintText(ctx: CanvasRenderingContext2D, command: PaintTextCommand): void {
...
function paintRuby(ctx: CanvasRenderingContext2D, command: PaintRubyCommand): void {
  drawRubyFragment(ctx, {
    text: command.text,
    rect: command.rect,
    paint: command.paint,
  });
