...
pub(crate) struct ParsedStyleChapterInput<'a> {
    ...
    pub stylesheet_hrefs: Option<&'a [String]>,
    pub embedded_stylesheets: Option<&'a [String]>,
}
...
fn summarize_style_for_parsed_chapter(
    ...
    let rules = build_chapter_rules(
        stylesheet_rules,
        chapter.stylesheet_hrefs,
        chapter.embedded_stylesheets,
...
pub(crate) fn build_chapter_rules(
    stylesheet_rules: &StylesheetRuleMap,
    link_hrefs: Option<&[String]>,
    embedded_stylesheets: Option<&[String]>,
    root_font_size: f64,
) -> Vec<CssRuleSummary> {
    let mut rules = link_hrefs.map_or_else(
        || {
            stylesheet_rules
                .iter()
        ...
        |hrefs| filter_rules_by_chapter_hrefs(stylesheet_rules, hrefs),
    );
    for css in embedded_stylesheets.unwrap_or_default() {
...
fn find_matching_stylesheet_keys<'a>(keys: &[&'a str], link_href: &str) -> Vec<&'a str> {
...
fn walk_sibling_nodes(
    ...
    for node in nodes {
        ...
        target.previous_sibling = previous_sibling.clone().map(Box::new);
...
fn resolve_nodes_with_siblings(
    ...
    for node in nodes {
        ...
        if let Some(info) = sibling_info {
            ...
            target.previous_sibling = info.previous_sibling.map(Box::new);
            previous_sibling = Some(target);
        }
...
struct SiblingInfo {
    sibling_index: usize,
    sibling_count: usize,
    previous_sibling: Option<SelectorTarget>,
}
...
fn apply_runtime_element_defaults(mut style: Map<String, Value>, tag: &str) -> Map<String, Value> {
    if tag == "img" {
...
fn insert_border(style: &mut Map<String, Value>, key: &str, width: f64, color: &str, kind: &str) {
    style.insert(
        key.to_owned(),
...
const DEFAULT_UA_STYLESHEET: &str = r#"
...
mod tests {
    ...
    use crate::{css::parse_css_rules, xhtml::parse_xhtml};
    ...
    fn chapter_body_style_applies_presentational_class_and_inline_attrs() {
        ...
        .expect("xhtml parses");
        let rules = parse_css_rules("body.cover { background-color: #1EDCF0; color: #abcdef; }");
    ...
    #[test]
    fn chapter_rules_preserve_missing_empty_embedded_and_ambiguous_href_semantics() {
        ...
        let all = build_chapter_rules(&stylesheets, None, None, 18.0);
        assert_eq!(
            all.iter()
        ...
        assert!(build_chapter_rules(&stylesheets, Some(&[]), None, 18.0).is_empty());
        assert!(
            build_chapter_rules(&stylesheets, Some(&["../main.css".to_owned()]), None, 18.0,)
                .is_empty()
        );

        let selected = build_chapter_rules(
            &stylesheets,
            Some(&["../Styles/only.css?cache=1".to_owned()]),
            Some(&[".embedded { margin-left: 1rem; }".to_owned()]),
            18.0,
        );
        assert_eq!(
            selected
        ...
        assert_eq!(selected[1].declarations.get("marginLeft"), Some(&json!(18)));
    }
