...
impl FormattingContext for ParleyInlineContext {
    fn layout(
        ...
        let line_debug = std::env::var_os("RITO_LINE_DEBUG").is_some();
            ...
            // the ceiling of the running float end on the 1/64 grid
            // (Range-measured: adjacent 15.2px spans put the second run
            // at 45.609375 = ceil64(45.6), where the raw float
            // continuation sits at 45.6); interiors keep the float
            // accumulation from the anchored start.
                                ...
                                let run_x = match &mut natural_item_track {
                                    ...
                                    Some((item, truth_start, engine_start)) => {
                                        if *item != item_index {
                                            ...
                                            let truth = ((*truth_start + advance) * 64.0).ceil()
                                                / 64.0;
                                            *item = item_index;
                                            *truth_start = truth;
                                            *engine_start = run_x;
