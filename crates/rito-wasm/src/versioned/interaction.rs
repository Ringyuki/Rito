...
use crate::{
    wire::{
        ...
        parse_text_range_request, parse_text_range_to_point_request,
        parse_text_selection_movement_request, serialize_json,
    },
...
impl WasmRuntimeDocument {
    ...
    pub fn resolve_text_range_to_point_at_revision_json(
        ...
        let request = parse_text_range_to_point_request(request_json)?;
        let response = self
            .document
    ...
    pub fn resolve_text_selection_movement_at_revision_json(
        &self,
        revision_id: &str,
        revision_version: u32,
        request_json: &str,
    ) -> Result<String, WasmRuntimeError> {
        let request = parse_text_selection_movement_request(request_json)?;
        let response = self
            .document
