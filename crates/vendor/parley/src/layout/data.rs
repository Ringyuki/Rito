...
impl<B: Brush> LayoutData<B> {
    ...
    pub(crate) fn calculate_content_widths(&self) -> ContentWidths {
        fn whitespace_advance(cluster: Option<&ClusterData>) -> f32 {
            // Rito: the preferred-width trim drops only a plain SPACE.
            // A trailing U+00A0 stays in the measure, as the browser
            ...
            match item.kind {
                LayoutItemKind::TextRun => {
                    ...
                    for cluster in clusters {
                        ...
                        if !is_rtl {
                            // Rito: a newline cluster must not shadow the
                            // segment's last real cluster — the mandatory
